-- Fix search_thoughts to look for anchored_at IS NOT NULL (more reliable than state field)
-- Also, allow searching across thoughts that have content regardless of state
...
AS $$
  ...
  WHERE wt.user_id = COALESCE(search_user_id, auth.uid())
    AND wt.anchored_at IS NOT NULL  -- Use anchored_at instead of state field
