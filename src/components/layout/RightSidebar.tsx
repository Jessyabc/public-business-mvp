...
export function RightSidebar({
  ...
  const fetchFeeds = async () => {
    try {
      ...
      const {
        data: ideas
      } = await supabase.from('open_ideas_intake').select('id, text, created_at, status').order('created_at', {
        ascending: false
      }).limit(5);
  ...
  if (variant === 'feed') {
    ...
    const renderOpenIdeas = () => {
      ...
      return <div className="space-y-3">
          {openIdeas.map(idea => <GlassCard key={idea.id} padding="sm" className="border-white/15 bg-white/5 backdrop-blur">
              <p className="text-sm text-white/85">{idea.text}</p>
              <div className="flex justify-between items-center mt-2">
                <p className="text-[11px] uppercase tracking-[0.3em] text-white/50">
                  {new Date(idea.created_at).toLocaleDateString()}
                </p>
                <span className={`text-xs px-2 py-0.5 rounded ${idea.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                  {idea.status}
                </span>
              </div>
            </GlassCard>)}
    ...
    return <div className="flex h-full flex-col">
        <div className="rounded-full bg-white/5 p-1 text-xs uppercase tracking-[0.3em] text-white/60 px-0 py-[5px]">
          <div className="grid grid-cols-2 gap-1">
            {[{
            id: 'breadcrumbs' as const,
            label: 'Breadcrumbs'
          }, {
            id: 'openIdeas' as const,
            label: 'Open Ideas'
          }].map(tab => <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`rounded-full px-3 py-1 text-xs transition ${activeTab === tab.id ? 'bg-white/90 text-black' : 'text-white/70'}`}>
                {tab.label}
              </button>)}
          </div>
        </div>
