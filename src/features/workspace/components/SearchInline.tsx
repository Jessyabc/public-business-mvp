 ...
 export function SearchInline({ className }: SearchInlineProps) {
   ...
   return (
     <div className={cn("search-inline relative", className)}>
       {/* Search trigger button */}
       {!isOpen && (
         <button
           onClick={() => setIsOpen(true)}
           className="flex items-center gap-2 px-3 py-1.5 rounded-full transition-all opacity-50 hover:opacity-100"
           style={{ 
       ...
       <AnimatePresence>
         {isOpen && (
           ...
           >
             ...
             >
               {isSearching ? (
                 <Loader2 className="w-3.5 h-3.5 animate-spin" style={{ color: PB_BLUE }} />
               ) : (
                 <Search className="w-3.5 h-3.5" style={{ color: PB_BLUE }} />
               )}
               <input
