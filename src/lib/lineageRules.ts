...
export type RelationKind = 'origin' | 'reply' | 'quote' | 'cross_link';

// Legacy support: map old types to new
export function normalizeRelationType(oldType: string): RelationKind {
  switch (oldType) {
    case 'hard': return 'origin';
    case 'soft': return 'cross_link';
    case 'biz_in':
    case 'biz_out': return 'cross_link';
    default: return oldType as RelationKind;
  }
}
